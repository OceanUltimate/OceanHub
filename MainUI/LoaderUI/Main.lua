...
local SUPABASE_URL = "https://diepaphbtpepwzqdaujq.supabase.co"
local SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImRpZXBhcGhidHBlcHd6cWRhdWpxIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODI5OTk5MjIsImV4cCI6MjA5ODU3NTkyMn0.RBiaEWZD5D0H50X5YFJd7F-GaYXBG4nPcHqaLr-1KCI"
...
-- ── KEY VERIFICATION (executor-compatible, no HttpService:PostAsync) ──
-- Uses request() / http.request() / syn.request() — standard exploit HTTP
local function verifyKey(key)
    local body = '{"input_key":"'..key..'","player_name":"'..LocalPlayer.Name..'"}'
    local hdrs = {
        ["Content-Type"]  = "application/json",
        ["apikey"]        = SUPABASE_KEY,
        ["Authorization"] = "Bearer "..SUPABASE_KEY,
    }
    local endpoint = SUPABASE_URL.."/rest/v1/rpc/validate_key"

    -- Try request() (most executors: Synapse X, KRNL, Fluxus, Solara, etc.)
    local reqFn = request or http_request or (syn and syn.request) or (http and http.request)
    if not reqFn then
        return false, "HTTP not available in this executor"
    end
    ...
    local data = pcall(function() return HttpService:JSONDecode(res.Body or "") end)
    -- JSONDecode in pcall form returns ok,val — redo properly
    local decodeOk, decoded = pcall(function() return HttpService:JSONDecode(res.Body or "") end)
    if not decodeOk or type(decoded) ~= "table" then
        return false, "Bad response from server"
    end
    if decoded.valid == true then
        return true, decoded.message or "Key valid!", decoded.tier
    end
    return false, decoded.message or "Invalid or expired key"
end

-- ── HELPERS ───────────────────────────────────────────────────
local function mkCorner(r, p) local c=Instance.new("UICorner"); c.CornerRadius=UDim.new(0,r or 8); c.Parent=p end
local function mkStroke(col,th,tr,p) local s=Instance.new("UIStroke"); s.Color=col; s.Thickness=th or 1.5; s.Transparency=tr or 0.4; s.Parent=p end
local function mkPad(t,b,l,r,p) local x=Instance.new("UIPadding"); x.PaddingTop=UDim.new(0,t); x.PaddingBottom=UDim.new(0,b); x.PaddingLeft=UDim.new(0,l); x.PaddingRight=UDim.new(0,r); x.Parent=p end
local function tw(obj,props,dur,style,dir) TweenService:Create(obj,TweenInfo.new(dur or 0.25,style or Enum.EasingStyle.Quad,dir or Enum.EasingDirection.Out),props):Play() end
local function mkLabel(parent,text,sz,col,font,align,z)
    local l=Instance.new("TextLabel"); l.BackgroundTransparency=1; l.Text=text; l.TextColor3=col or CFG.TEXT
    l.TextSize=sz or 14; l.Font=font or Enum.Font.Gotham; l.TextXAlignment=align or Enum.TextXAlignment.Left
    l.ZIndex=z or 5; l.Parent=parent; return l
end
...
Gui.Name="OceanHubLoaderUI"; Gui.ResetOnSpawn=false
Gui.ZIndexBehavior=Enum.ZIndexBehavior.Global; Gui.IgnoreGuiInset=true; Gui.Parent=PlayerGui

local Overlay = Instance.new("Frame")
Overlay.Size=UDim2.fromScale(1,1); Overlay.BackgroundColor3=Color3.fromRGB(3,10,20)
Overlay.BackgroundTransparency=1; Overlay.ZIndex=10; Overlay.Active=false; Overlay.Parent=Gui

-- ── CARD ──────────────────────────────────────────────────────
local Card = Instance.new("Frame")
Card.Name="Card"; Card.Size=UDim2.new(0,420,0,340); Card.Position=UDim2.fromScale(0.5,0.5)
Card.AnchorPoint=Vector2.new(0.5,0.5); Card.BackgroundColor3=CFG.CARD
Card.BackgroundTransparency=1; Card.ZIndex=20; Card.Active=true; Card.Parent=Gui
mkCorner(20,Card); mkStroke(CFG.ACCENT,1.5,0.35,Card)

local CloseBtn=Instance.new("TextButton")
CloseBtn.Size=UDim2.new(0,28,0,28); CloseBtn.Position=UDim2.new(1,-10,0,-10)
CloseBtn.AnchorPoint=Vector2.new(0.5,0.5); CloseBtn.BackgroundColor3=CFG.FREE_COL
CloseBtn.BackgroundTransparency=0.3; CloseBtn.Text="✕"; CloseBtn.TextColor3=Color3.fromRGB(255,200,200)
CloseBtn.TextSize=14; CloseBtn.Font=Enum.Font.GothamBold; CloseBtn.ZIndex=50; CloseBtn.Parent=Card
mkCorner(14,CloseBtn)

local Ico=Instance.new("ImageLabel")
Ico.Size=UDim2.new(0,52,0,52); Ico.Position=UDim2.new(0,16,0,18)
Ico.BackgroundColor3=Color3.fromRGB(15,70,160); Ico.Image=CFG.ICON
Ico.ScaleType=Enum.ScaleType.Fit; Ico.ZIndex=25; Ico.Parent=Card
...
HubTitle.Size=UDim2.new(1,-90,0,30); HubTitle.Position=UDim2.new(0,80,0,22)

local HubSub=mkLabel(Card,"LOADER "..CFG.VERSION,13,CFG.MUTED,Enum.Font.GothamMedium,Enum.TextXAlignment.Left,25)
HubSub.Size=UDim2.new(1,-90,0,18); HubSub.Position=UDim2.new(0,80,0,54)

local Div=Instance.new("Frame")
Div.Size=UDim2.new(1,-32,0,1); Div.Position=UDim2.new(0,16,0,90)
Div.BackgroundColor3=CFG.ACCENT; Div.BackgroundTransparency=0.65; Div.ZIndex=22; Div.Parent=Card

local ChooseLbl=mkLabel(Card,"Choose your access type",14,CFG.MUTED,Enum.Font.Gotham,Enum.TextXAlignment.Left,22)
ChooseLbl.Size=UDim2.new(1,-32,0,22); ChooseLbl.Position=UDim2.new(0,16,0,102)

local function makeAccessBtn(posX,bgCol,accentCol,titleTxt,descTxt,textCol)
    local btn=Instance.new("TextButton")
    btn.Size=UDim2.new(0,177,0,110); btn.Position=UDim2.new(0,posX,0,132)
    btn.BackgroundColor3=bgCol; btn.BackgroundTransparency=0.4; btn.Text=""
    btn.ZIndex=23; btn.Active=true; btn.Parent=Card
    ...
    dl.Size=UDim2.new(1,-8,0,18); dl.Position=UDim2.new(0,4,0,66); dl.TextTransparency=0.3
...
local BtnFree    = makeAccessBtn(16,  Color3.fromRGB(100,20,30), CFG.FREE_COL, "🆓 Free",    "Free scripts",  Color3.fromRGB(255,200,205))
local BtnPremium = makeAccessBtn(207, CFG.PREM_BG,               CFG.PREM_COL, "👑 Premium", "Key required",  CFG.PREM_TEXT)

local Footer=mkLabel(Card,"OceanUltimate  •  github.com/OceanUltimate/OceanHub",10,Color3.fromRGB(40,65,100),Enum.Font.Gotham,Enum.TextXAlignment.Center,21)
Footer.Size=UDim2.new(1,0,0,16); Footer.Position=UDim2.new(0,0,1,-20)

-- ── TOAST ─────────────────────────────────────────────────────
local function showToast(msg, color)
    local t=Instance.new("Frame")
    t.Size=UDim2.new(0,310,0,44); t.Position=UDim2.new(0.5,0,1,70)
    t.AnchorPoint=Vector2.new(0.5,1); t.BackgroundColor3=color or CFG.ACCENT
...
local function loadScript(path, toastMsg, toastCol)
    ...
    task.delay(0.4, function()
        ...
        local old=PlayerGui:FindFirstChild("OceanHubLoader"); if old then old:Destroy() end
        local ok,err=pcall(function()
            local src=safeSrc(R..path)
...
local function makeModal(titleTxt, modalH, accentCol, titleCol)
    ...
    modal.Size=UDim2.new(0,380,0,modalH); modal.Position=UDim2.fromScale(0.5,0.5)
    modal.AnchorPoint=Vector2.new(0.5,0.5); modal.BackgroundColor3=Color3.fromRGB(5,14,28)
    modal.BackgroundTransparency=0.05; modal.ZIndex=100; modal.Visible=false
    modal.Active=true; modal.Parent=Gui
    ...
    divM.BackgroundColor3=ac; divM.BackgroundTransparency=0.65; divM.ZIndex=102; divM.Parent=modal

    local closeM=Instance.new("TextButton")
    closeM.Size=UDim2.new(0,26,0,26); closeM.Position=UDim2.new(1,-10,0,-10)
    closeM.AnchorPoint=Vector2.new(0.5,0.5); closeM.BackgroundColor3=CFG.FREE_COL
    closeM.BackgroundTransparency=0.3; closeM.Text="✕"; closeM.TextColor3=Color3.fromRGB(255,200,200)
    closeM.TextSize=14; closeM.Font=Enum.Font.GothamBold; closeM.ZIndex=120; closeM.Parent=modal
    mkCorner(13,closeM)

    local function openModal()
        modal.Size=UDim2.new(0,380,0,0); modal.BackgroundTransparency=1; modal.Visible=true
        tw(modal,{Size=UDim2.new(0,380,0,modalH),BackgroundTransparency=0.05},0.25,Enum.EasingStyle.Back,Enum.EasingDirection.Out)
    end
...
local function buildGameList(parent, games, startY, accentCol, onSelect)
    ...
    scroll.Size=UDim2.new(1,-32,1,-(startY+14)); scroll.Position=UDim2.new(0,16,0,startY)
    scroll.BackgroundTransparency=1; scroll.BorderSizePixel=0
    scroll.ScrollBarThickness=3; scroll.ScrollBarImageColor3=ac; scroll.ZIndex=105
    ...
    layout.Padding=UDim.new(0,8); layout.Parent=scroll
    for i,g in ipairs(games) do
        local btn=Instance.new("TextButton")
        btn.Size=UDim2.new(1,0,0,44); btn.BackgroundColor3=ac; btn.BackgroundTransparency=0.6
        btn.Text=g.name; btn.TextColor3=textCol; btn.TextSize=16; btn.Font=Enum.Font.GothamMedium
        btn.TextXAlignment=Enum.TextXAlignment.Left; btn.ZIndex=110; btn.LayoutOrder=i; btn.Parent=scroll
...
buildGameList(FreeModal, CFG.GAMES_FREE, 64, CFG.ACCENT, function(g)
    loadScript("MainScript/Script/MenuFree/FreeScript/"..g.folder.."/Main.lua",
        "🆓 Loading "..g.name.."...", CFG.FREE_COL)
end)
...
local KeyError=mkLabel(KeyModal,"Invalid or expired key!",13,Color3.fromRGB(255,110,110),Enum.Font.Gotham,Enum.TextXAlignment.Left,115)
KeyError.Size=UDim2.new(1,-32,0,18); KeyError.Position=UDim2.new(0,16,0,112); KeyError.Visible=false
...
VerifyBtn.MouseButton1Click:Connect(function()
    ...
    if key=="" then KeyError.Text="Key cannot be empty!"; KeyError.Visible=true; return end
        ...
        if valid then
            ...
            task.delay(0.5, function()
                ...
                local ok,e=pcall(function()
                    local src=safeSrc(R.."MainScript/Script/MenuPremium/MenuPremiumLoader.lua")
...
CloseBtn.MouseButton1Click:Connect(function()
    tw(Card,{BackgroundTransparency=1},0.3); tw(Overlay,{BackgroundTransparency=1},0.3)

--[[
    OceanHub - Rivals Premium Script (FULL FEATURED)
    Task: Smooth Aimbot, Silent Aim, FOV, Wallcheck, TargetPart, Triggerbot, ESP Box, ESP Tracers, ESP Name, Speed, Jump, Premium Customizer
]]
...
welcomeCard:AddBanner({ Text = "Kamu menggunakan Rivals Premium Script.", Icon = "✦" })
welcomeCard:AddKeyVal({ Key = "Game Terdeteksi", Value = "Rivals", Color = Color3.fromRGB(56, 189, 248) })
...
CombatTab:AddToggle({ Name = "Smooth Aimbot", Keybind = "E", Default = false, Callback = function(val) Settings.Aimbot = val end })
CombatTab:AddToggle({ Name = "Silent Aim", Keybind = "R", Default = false, Callback = function(val) Settings.SilentAim = val end })
CombatTab:AddToggle({ Name = "Triggerbot", Keybind = "F", Default = false, Callback = function(val) Settings.Triggerbot = val end })
...
CombatTab:AddSlider({ Name = "Smoothness", Min = 1, Max = 20, Default = 8, Callback = function(val) Settings.Smoothness = val end })
...
VisualTab:AddToggle({ Name = "ESP Player Names", Default = false, Callback = function(val) Settings.ESPNames = val end })
...
OceanLibrary:Notify({ Title = "OceanHub VIP", Content = "Rivals Premium Script loaded with 12+ Features!", Duration = 5 })
